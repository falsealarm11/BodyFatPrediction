...
model = pickle.load(open('bodyfat.pkl', 'rb'))
app=Flask(__name__)
...
def predict() :
    ...
    prediction = model.predict(pd.DataFrame([[density,age,height,neck,ankle,biceps,forearm,wrist]], columns= ['Density', 'Age', 'Height', 'Neck', 'Ankle', 'Biceps',
       'Forearm', 'Wrist']))
    
    prediction = np.round(prediction,2)
    
    
    return render_template('fat_predict.html', prediction_text ="is {}".format(prediction))
